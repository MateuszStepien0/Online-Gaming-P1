...
void Packet::Append(const PlayerData& updateData)
{
	Append((const char*)&updateData, sizeof(PlayerData));
}

void Packet::Append(const GameState& state)
{
	Append((const char*)&state, sizeof(GameState));
}

void Packet::Append(const StartData& startData)
{
	Append((const char*)&startData, sizeof(StartData));
}

void Packet::Append(const EndData& endData)
{
	Append((const char*)&endData, sizeof(EndData));
}

void Packet::Append(const std::array<sf::Vector2f, 3> positions)
{
	Append((const char*)&positions[0], sizeof(positions));
}
